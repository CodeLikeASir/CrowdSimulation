...
void construct_triangle(GLfloat* triangle, float2 pos, float2 dir)
{
	...
	float2 dirSize = make_float2(1.5f * TRIANGLE_SIZE * dir.x, 1.5f * TRIANGLE_SIZE * dir.y);
	triangle[0] = pos.x + 1.2f * TRIANGLE_SIZE * dir.x;
	triangle[1] = pos.y + 1.2f * TRIANGLE_SIZE * dir.y;
	triangle[2] = 0.f;

	triangle[3] = pos.x - dirSize.x + TRIANGLE_SIZE * orth_dir.x;
	triangle[4] = pos.y - dirSize.y + TRIANGLE_SIZE * orth_dir.y;
	triangle[5] = 0.f;

	triangle[6] = pos.x - dirSize.x - TRIANGLE_SIZE * orth_dir.x;
	triangle[7] = pos.y - dirSize.y - TRIANGLE_SIZE * orth_dir.y;
	triangle[8] = 0.f;
}
...
{
	std::fill_n(vertices, SPAWNED_ACTORS * 9, 0.f);
...
int main()
{
	...
	bool limitSimulation = SIMULATION_STEPS > 0;
	int remainingUpdates = SIMULATION_STEPS;
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		if (limitSimulation && --remainingUpdates <= 0)
			break;
	}
